...
-(void) setInteractionEnabledOrNotOnButtons:(BOOL)enabled
{
    for(int i=0;i<3;i++) {
        for(int j=0;j<3;j++) {
            if([self.gameObj.cellArray[i][j] status] == none) {
...
-(void) fillButtonArray {
    ...
    for (NSInteger i=0; i<3; i++) {
        ...
        for (NSInteger j=0; j<3; j++) {
            ...
            TDTCellPosition *position = [self.gameObj.cellArray[i][j] cellPosition];
            TDTCellButton *buttonToAddAtPosition = [[TDTCellButton alloc] initWithFrame:cellRect withColor:UNSELECTED_CELL_COLOR withPosition:position];
            [buttonToAddAtPosition addTarget:self action:@selector(buttonPressed:) forControlEvents:UIControlEventTouchUpInside];
            [rowArray addObject:buttonToAddAtPosition];
        }
